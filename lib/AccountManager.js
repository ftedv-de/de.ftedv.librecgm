...
module.exports = class AccountManager {
  ...
  async registerAccount({
    ...
    if (!result.connections.some(connection =>
      (connection.patientId || connection.id) === patientId)) {
  ...
  async registerAccountOnce({ credentials, auth, connections }) {
    ...
    let account = [...this.accounts.values()].find(item =>
      this.accountKey(item.credentials) === this.accountKey(normalizedCredentials));
  ...
  subscribe(accountId, patientId, subscriber) {
    ...
    if (cached) {
      Promise.resolve(subscriber.onReading(cached)).catch(err =>
        this.homey.app.error('Delivering cached glucose reading failed:', err));
    }
  ...
  schedule(account, delay) {
    ...
    account.timer = this.homey.setTimeout(() => {
      account.timer = null;
      this.poll(account).catch(err =>
        this.homey.app.error('Account polling failed:', err));
    }, delay);
  }
    ...
    try {
      ...
      for (const subscribers of account.subscribers.values()) {
        for (const subscriber of subscribers) {
          Promise.resolve(subscriber.onError?.(err)).catch(error =>
            this.homey.app.error('Delivering polling error failed:', error));
        }
      }
