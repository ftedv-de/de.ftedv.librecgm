...
module.exports = class LibreViewClient {
  ...
  async getConnectionReading(patientId, options = {}) {
    ...
    const reading = LibreViewClient.normalizeReading({
      measurement,
      connection,
    ...
    return {
      ...reading,
      history: LibreViewClient.normalizeGraphData(data?.graphData),
    };
  }
  ...
  async request(path, { method = 'GET', auth = true, body = undefined } = {}) {
    ...
    try {
      ...
      if (err?.name === 'AbortError' || err?.name === 'TimeoutError') {
        throw new Error(`LibreLinkUp request timed out after ${this.timeoutMs} ms`);
      }
  ...
  static normalizeReading({
    ...
    return {
      ...
      sensorSerialNumber:
        connection?.sensor?.sn
        ?? activeSensors?.[0]?.sensor?.sn
        ?? null,
    };
  }
  ...
  static calculateSensorExpiryHours(sensor, activeSensors = [], sensorLifetimeDays = 16) {
    ...
    const expiresAtMs = activatedAtSeconds * 1000
      + sensorLifetimeDays * 24 * 60 * 60 * 1000;

    return Math.max(0, Math.round((expiresAtMs - Date.now()) / 3600000));
  }

  static normalizeGraphData(graphData = []) {
    if (!Array.isArray(graphData)) return [];

    return graphData.map((measurement) => {
      const valueMgDl = Number(measurement?.ValueInMgPerDl);
      const timestamp = measurement?.Timestamp ?? measurement?.FactoryTimestamp;

      if (!Number.isFinite(valueMgDl) || !LibreViewClient.parseLibreDate(timestamp)) {
        return null;
      }
      ...
      return {
        timestamp,
        valueMgDl,
        valueMmol: LibreViewClient.mgDlToMmol(valueMgDl),
        trend: LibreViewClient.trendArrowToText(measurement.TrendArrow),
        trendArrow: measurement.TrendArrow ?? null,
      };
    }).filter(Boolean);
